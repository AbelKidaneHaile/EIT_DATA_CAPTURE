...
def mysidebar():
    ...
    try:
        ...
        if time_val < 0:
            ...
            time_val = 1
            st.sidebar.warning(f"Time must not be above {MAX_PROGRESS}. This could lead to overinflation.")
    except ValueError:
    ...
    if st.sidebar.button("Inflate"):
        dummy_function()
        st.session_state.progress = min(st.session_state.progress + time_val, MAX_PROGRESS)
...
def mymainpage():
    ...
    st.progress(progress_percent, text=f"Progress: {st.session_state.progress} / {MAX_PROGRESS}")
